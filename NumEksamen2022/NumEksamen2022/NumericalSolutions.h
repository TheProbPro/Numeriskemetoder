...
namespace NumericalSolutions
{
	...
	void printoutTable(VecDoub& a, VecDoub& fcalc, double accuracy) {
		...
		std::cout << std::setw(8) << "i" << " | " << std::setw(9) << "a[i]" << " | " <<
			std::setw(8) << "a[i-1] - a[i]" << " | " <<
			std::setw(8) << "Rich-alp^k" << " | " <<
			std::setw(8) << "Rich-fejl" << " | " <<
			std::setw(8) << "Antal f-beregninger" << std::endl;
		...
		for (size_t i = 0; i < a.size(); i++) {
			if (i == 0) {
				...
				std::cout << std::setw(8) << i << " | " <<
					std::setw(8) << a[i] << " | " <<
					std::setw(14) << abs(a[i] - a[i - 1]) << "| " <<
					std::setw(10) << " ******* " << " | " <<
					std::setw(8) << " ******* " << " | " <<
					std::setw(8) << fcalc[i] << std::endl;
			}
			else if (i > 6 && abs(a[i] - a[i - 1]) < accuracy) {
				std::cout << "error: " << abs(a[i] - a[i - 1]) << std::endl;
				break;
			}
			else {
				std::cout << std::setw(8) << i << " | " <<
					std::setw(8) << a[i] << " | " <<
					std::setw(12) << abs(a[i] - a[i - 1]) << "  |  " <<
	...
	// Midpoint method
	VecDoub Midpoint(VecDoub(*f)(VecDoub guess), VecDoub guess, int a, int b, VecDoub& h, VecDoub& ans) {
		// Declaration of variables
		VecDoub Fxy0(5, 0.0), Fxy1(5, 0.0), FxyNew(5, 0.0), FxyHalf(5, 0.0), xy(5, 0.0);
		double stepsize = 0;
		...
		for (int i = 0; i < ans.size(); ++i) {
			...
			for (double j = a; j < b; j = j + stepsize) {
				...
				for (int n = 0; n < xy.size(); ++n) {
					FxyHalf[n] = xy[n] + 0.5 * stepsize * Fxy0[n];
				}
				Fxy1 = f(xy);
				for (int n = 0; n < xy.size(); ++n) {
					FxyNew[n] = xy[n] + stepsize * Fxy1[n];
				}
				// Setting the new x and y values
				for (int n = 0; n < xy.size(); ++n) {
					xy[n] = FxyNew[n];
	...
	MatDoub PlotMidpoint(VecDoub(*f)(VecDoub guess), VecDoub guess, int a, int b, VecDoub& h, MatDoub& ans) {
		// Declaration of variables
		VecDoub Fxy0(5, 0.0), Fxy1(5, 0.0), FxyNew(5, 0.0), FxyHalf(5, 0.0), xy(5, 0.0);
		double stepsize = 0;
